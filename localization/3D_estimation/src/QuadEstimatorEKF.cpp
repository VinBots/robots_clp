...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  //create a rotation matrix based on current Euler angles
    Mat3x3F rotation;
    rotation(0,0) = 1;
    ...
    // a_dot is a 3-element vector for phi_dot, theta-dot, psi_dot
    
    V3F a_dot = rotation * gyro;

    // Integration
    float predictedRoll = rollEst + dtIMU * a_dot.x;
    float predictedPitch = pitchEst + dtIMU * a_dot.y;
    ekfState(6) += dtIMU * a_dot.z;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    predictedState(5) = curState(5) + acc_world.z * dt - CONST_GRAVITY * dt;
