...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&lane,&lane_change_wp,&ego](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //double car_x = j[1]["x"];
            ...
            int steps_ahead_refresh = 50;
          	int transition_steps = 0; // additional time needed for changing lanes
			string new_state;
          	int prev_size = previous_path_x.size();
			//std::cout<<"Previous Path Size = "<<prev_size<<"\n";
          	...
          	else
          	{
				...
				next_wp = NextWaypoint(ref_x,ref_y,ref_yaw,map_waypoints_x,map_waypoints_y,map_waypoints_dx,map_waypoints_dy);
                //std::cout<<"Next Waypoint = "<<next_wp<<"\n";
          ...
          if (ego.vehicles_ahead[ego.lane].horizon_s!=-1 && ego.vehicles_ahead[ego.lane].horizon_s - ego.horizon_s < 30.0)
            {
          	...
          	//std::cout<<"My lane is "<<ego.lane<<" vs. fastest lane = "<<ego.best_lane()<<"\n";
            //std::cout <<"Previous size = "<<prev_size<<"\n";
          
            //ego.set_speed();
          	
            
            /*
            std::cout<<"My state is "<<ego.state<<" and my possible successor states are ";
            for (int k = 0;k<states.size();++k)
            {
          	...
          	std::cout<<"The fastest lane is "<<ego.best_lane()<<" with a speed of "<< ego.vehicles_ahead[ego.best_lane()].v<<"\n";
          
          
            for (int i = 0;i<3;++i)
            {
              if (ego.vehicles_ahead[i].v!=-1)
              {
                std::cout<<"Lane "<<i<<" - Speed: "<<ego.vehicles_ahead[i].v<<"; Distance Ahead a "<<ego.vehicles_ahead[i].s - ego.s<<" \n";
              }
                          }
            
            // update states
            if (new_state.compare(old_state)!=0)
            {
              
          	std::cout<<"State transition from "<<old_state<<" to "<<new_state<<" \n";
            
            std::cout<<"In-lane speed = "<<ego.vehicles_ahead[old_lane].v<<" vs. lane speed: "<<ego.vehicles_ahead[new_lane].v<<"\n";
            }
          */
            //std::cout<<"State : "<<ego.state<<"; ";
            if (ego.state.compare("LCR") ==0 || ego.state.compare("LCL")==0)
              {
                steps_ahead_refresh += transition_steps;
              }
            ...
            lane = ego.lane;
          	...
          	//std::cout<<"Print Ptsx,Ptsy before Spine - Size: "<<ptsx.size()<<" points \n";
